	...
	public static void main(String[] args) {
		...
		do {
			...
			case 2:
				...
				if(a != null) {
					for (i = 0; i < a.length && a[i] != null; i++) {
						if(res1.equalsIgnoreCase(a[i].getHabitat())) {
							System.out.println("Animale trovato : " + a[i].toString());
							trovato = true;
						}
						if(trovato) {
							aCopia = v.duplica(a[i]);
				...
				break;
			case 3:
				res1 = v.leggiStringa("Cerca per habitat");
				if(a != null) {
					for (i = 0; i < a.length && a[i] != null; i++) {
						if(res1.equalsIgnoreCase(a[i].getHabitat())) {
							System.out.println("Animale trovato : " + a[i].toString());
						...
						if(trovato) {
							String modificaAnimale = v.leggiStringa("Sei sicuro di voler eliminare l'animale? si/no");
							if(modificaAnimale.equalsIgnoreCase("si"))
								a[i] = null;
							break;
						}
