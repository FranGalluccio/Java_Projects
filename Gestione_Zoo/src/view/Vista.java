...
public class Vista implements IVista{
	...
	public void modificaAnimale(Animale a) {
		if (a instanceof Cane) {
			...
			System.out.println("Velocità: "	+ ((Uccello) a).getVelocita());
			((Uccello) a).setVelocita(leggiIntero("Inserisci nuova velocità: "));
		}
		else if (a instanceof DelfinoCurioso) {
			modificaBase(a);
	...
	public void stampaScheda(Animale[] a) {
		for (int i = 0; i < a.length; i++) {
			if(a[i] instanceof Cane) {
				...
				System.out.println("Velocità: " + ((Uccello)a[i]).getVelocita());
			}
